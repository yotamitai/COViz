...
def remove_attention_check (df):
    ...
    #index_check = df[(df['sat HL check'] != 2)].index
...
def number_of_participants_per_condition(df):
    ...
    num_CH_H = len(df[df.condition == 'SUM'])
    ...
    print('number of participants in condition "CH_H" ', num_CH_H)
    return (num_CH,num_CRD,num_RD,num_CH_H)
...
def duration_per_condition(df):
    ...
    mean_duration_CH_H = df_CH_H['Q_TotalDuration'].mean()
    print("mean_duration_CH_H:",mean_duration_CH_H)
    print("mean_duration_CH:", mean_duration_CH)
    print("mean_duration_CRD:", mean_duration_CRD)
    print("mean_duration_RD:", mean_duration_RD)
...
def create_bar_Agent_by_condition(q_df):
    ...
    q_df['Agent 1']= q_df[['g1_CLRL_RL_vs_HS', 'g1_CLRL_RL_vs_CL', 'g1_CLRL_HS_vs_CL',
                        'g2_CLRL_RL_vs_HS','g2_CLRL_RL_vs_CL', 'g2_CLRL_HS_vs_CL',
                            'g3_CLRL_RL_vs_HS', 'g3_CLRL_RL_vs_CL','g3_CLRL_HS_vs_CL'
                            ]].sum(axis = 1)/3
    ...
    q_df = q_df.replace('CH', 'COViz')
    q_df = q_df.replace('CH_H', 'SUM')
    q_df = q_df.replace('CRD', 'CORD')
    agent_columns = ('Agent 1','Agent 2','Agent 3')
    agent_title = ('Change Lane Right Lane', 'Right Lane Change Lane', 'High Speed Change Lane')
    agent_title = ('Second study - Agent 1', 'Second study  - Agent 2', 'Second study - Agent 3')
    order_list = ['SUM', 'COViz', 'RD', 'CORD']
    for i in range(len(agent_columns)):
        y_axis = agent_columns[i]
        title_name = agent_title[i]
        sns.set(font_scale=1.5)
        graph = sns.barplot(x="condition", y=y_axis, data=q_df, estimator=np.mean, ci=95, capsize=.2, color='blue', order = order_list)
    ...
    if False:
        ...
        con_CH_H = q_df[q_df.condition == 'SUM']['Agent 1'].to_frame()
...
def temp_1(df_agents):
    #CH= COOviz
    #CRD= CORD
    df_agents = df_agents.replace('CH', 'COViz')
    df_agents = df_agents.replace('CRD', 'CORD')
    order_list = ['SUM','COViz', 'RD', 'CORD']
    ...
    graph = sns.barplot(x="Agent", y='score', hue='condition',
                        ...
                        data=agents,
                        estimator=np.mean, ci=95, capsize=.2, color='blue',hue_order = order_list)
    graph.set_xlabel("", fontsize=25)
    graph.set_ylabel("Correctness Rate", fontsize=25)
    graph.legend(loc=2, bbox_to_anchor=(1,1))
    graph.set_title('Study 1', fontsize=23)
    #graph.bar_label(graph.containers[0])
    plt.show()
...
def statistical_confidance(df_agents):
    ...
    con_FB = df_agents[df_agents.condition == 'SUM']['All Agents'].to_frame()
   ...
   #U_H_H_RD, p_H_H_RD = mannwhitneyu(con_H_RD, con_H_RD)
    ...
    U_H_RD_H, p_H_RD_H = mannwhitneyu(con_RD, con_FB)
    print("For FB and H+RD,", "the U1 is:", U_H_RD_H, "the p- value is:", p_H_RD_H)
    print("done")
...
def graph_for_sat(df_union):
    ...
    df_union=df_union.replace("CH_H",'SUM')
    df_union = df_union.replace('CH', 'COViz')
    df_union = df_union.replace('CRD', 'CORD')
    order_list = ['SUM', 'COViz', 'RD', 'CORD']
    #graph = sns.boxplot(data=df_union, x="condition", y='satisfaction', color='blue')
    graph = sns.barplot(x="condition", y='satisfaction', data=df_union, estimator=np.mean, ci=95, capsize=.2, color='blue', order = order_list)
    ...
    graph.axhline(3)
    #graph.bar_label(graph.containers[0])
    graph.set_title('Study 2 - satisfaction', fontsize=30)
    plt.show()
...
def marge_data():
    mturk_df_old = pd.read_csv('Batch_4903529_batch_results.csv')
    qualtrics_df_old = pd.read_csv('CRD+-+Number2_October+18,+2022_23.05.csv')
    mturk_df_5_6_old = pd.read_csv('Batch_4904951_batch_results.csv')
    qualtrics_df_5_6_old = pd.read_csv('CRD+-+Number2_October+19,+2022_22.39.csv')
    # adjust data
    qualtrics_df_old.drop(axis=0, labels=[0, 1], inplace=True)
    qualtrics_df_old = qualtrics_df_old.rename(columns={'Random ID': 'Answer.surveycode'})
    mturk_df_old['Answer.surveycode'] = mturk_df_old['Answer.surveycode'].astype(str)
    qualtrics_df_5_6_old.drop(axis=0, labels=[0, 1], inplace=True)
    qualtrics_df_5_6_old = qualtrics_df_5_6_old.rename(columns={'Random ID': 'Answer.surveycode'})
    mturk_df_5_6_old['Answer.surveycode'] = mturk_df_5_6_old['Answer.surveycode'].astype(str)
    df_old = qualtrics_df_old.merge(mturk_df_old, on='Answer.surveycode', how='inner')
    df_1_old = qualtrics_df_5_6_old.merge(mturk_df_5_6_old, on='Answer.surveycode', how='inner')

    df_union_old = pd.concat([df_1_old, df_old], axis=0, ignore_index=True)

    ##########NEW#########################

    mturk_df = pd.read_csv('Batch_5113443_batch_results_1_4.csv')
    qualtrics_df = pd.read_csv('CRD+-+Number+4_July+31,+2023_05.14.csv')
    mturk_df_5_6 = pd.read_csv('Batch_5113565_batch_results_2_4.csv')
    qualtrics_df_5_6 = pd.read_csv('CRD+-+Number+4_July+31,+2023_11.44.csv')
    # adjust data
    ...
    df_union_new = pd.concat([df_1, df], axis=0, ignore_index=True)
    df_union = pd.concat([df_union_new, df_union_old], axis=0, ignore_index=True)
    return df_union
